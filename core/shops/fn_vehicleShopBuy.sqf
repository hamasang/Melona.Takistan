...
if ((life_veh_shop select 0) == "med_air_hs") then {
    ...
    if (_spawnPoints isEqualType []) then {
        ...
        {
            if ((nearestObjects[(getMarkerPos _x),["Car","Ship","Air","Tank"],5]) isEqualTo []) exitWith {_spawnPoint = _x};
            true
        } count _spawnPoints;
    } else {
        if (nearestObjects[(getMarkerPos _spawnPoints),["Car","Ship","Air","Tank"],5] isEqualTo []) exitWith {_spawnPoint = _spawnPoints};
    };
};
