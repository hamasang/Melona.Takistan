...
if(side _robber != civilian) exitWith { hint "이 주유소를 털 수 없습니다!" };
if(_robber distance _shop > 5) exitWith { hint "그를 강탈하려면 계산원으로부터 5m 이내에 있어야합니다!" };

if !(_kassa) then { _kassa = 1000; };
if (_rip) exitWith { hint "Robbery already in progress!" };
if (vehicle player != _robber) exitWith { hint "차에서 내려!" };

if !(alive _robber) exitWith {};
if (currentWeapon _robber == "") exitWith { hint "하하, 협박 하지마! 여기서 나가세요!" };
if (_kassa == 0) exitWith { hint "계산대에 현금이 없습니다!" };
...
sleep 300;
_action = _shop addAction["주유소 털기",life_fnc_robstore];
_shop switchMove "";